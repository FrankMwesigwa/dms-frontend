...
const AgentSummary = () => {
  ...
  const loadOrders = async () => {
    ...
    try {
      const res = await API.get("/agent/cart", config);
      console.log("Agent Order Summary Fetch Backend ===>", res);
  ...
  const emptyUserCart = async (token) => {
    ...
    try {
      const res = await API.delete("/agent/order", config);
  ...
  const createOrder = async () => {
    ...
    try {
      const res = await API.post("/agent/order", config);
      console.log("Agent Orders Placed Backend ===>", res);
      setLoading(false);
      if (res.data.ok) {
        if (typeof window !== "undefined") localStorage.removeItem("agentcart");
        dispatch({
  ...
  return (
    <div class="card shadow-none border mb-0">
      <div class="card-body">
        <h4 class="card-title mb-4">Agent Order Summary</h4>
